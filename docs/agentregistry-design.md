...
```
registry/
  mcp-servers.yaml      # code-agent, enrichment-agent, ldap-mcp,
                         # servicenow-mcp, magic-fetch
...
kind: MCPServer
metadata:
  name: workbench/servicenow-mcp
  version: "1.0.0"
spec:
  title: ServiceNow API
  description: >
    Direct MCP bridge to ServiceNow REST API.  CRUD for incidents,
    users, CMDB CIs, and catalog items.
  packages:
    - registryType: oci
      identifier: ai-workbench-servicenow-mcp:latest
      runtimeHint: docker
      transport: { type: sse }
```
...
│ ITSM tools       │ servicenow-mcp     │ ServiceNow IntegHub     │
│ Directory        │ ldap-mcp           │ Entra ID / Graph API    │
