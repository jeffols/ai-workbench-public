...
You (CSV data in prompt) → Open WebUI
                             → Code Agent
                                 ...
                                 → Code Sandbox executes it:
                                     ...
                                     → mcpo REST → LDAP MCP (org directory)
                                     → mcpo REST → ServiceNow MCP (ITSM data)
                                     → df.to_excel() (export)
...
You (CSV data in prompt) → Open WebUI
                             → Enrichment Agent (orchestrator)
                                 → LDAP Agent → LDAP MCP (org directory)
                                 → ServiceNow Agent → ServiceNow MCP (ITSM data)
...
Code Sandbox, mcpo, LDAP MCP, ServiceNow MCP, Caddy, Phoenix.

**Services touched (Path B — agent-orchestrated):** Open WebUI, LiteLLM, Enrichment Agent, LDAP Agent, LDAP MCP,
ServiceNow Agent, ServiceNow MCP, Code Agent (sandbox), Caddy, Phoenix.

## Prerequisites

- Stack is running: `make up` (tools profile)
- LDAP credentials set: `LDAP_BIND_DN`, `LDAP_PASSWORD` in `.env`
- ServiceNow credentials set: `SERVICENOW_USERNAME`, `SERVICENOW_PASSWORD` in `.env`
...
> 2. For each login, calls the LDAP REST API to get their display name,
>    title, department, and manager
> 3. For each login, queries ServiceNow for open incidents assigned to them
...
> Check http://mcpo:8000/ldap/openapi.json and
> http://mcpo:8000/servicenow/openapi.json for the exact endpoints and parameters.
...
| Execution | Sandbox runs the code — calls mcpo → LDAP/ServiceNow → gets data |
...
Have 3–5 real login IDs (sAMAccountName values) from your org ready.
...
You can verify login IDs exist first by asking the LDAP Agent directly:
*"Look up user jdoe"*
...
>
> For each person:
> 1. Look up their display name, title, department, and manager in LDAP
> 2. Find any open incidents assigned to them in ServiceNow
> 3. Combine everything into a single Excel file with two sheets:
>    "Team Profile" (roster + LDAP data) and "Open Incidents"
...
| LDAP lookup | → LDAP Agent | Queries AD for each login — returns names, titles, departments, managers |
| ServiceNow query | → ServiceNow Agent | Searches for open incidents assigned to each person |
...
| Phoenix project | What to look for |
|----------------|-------------------|
| `enrichment-agent` | Top-level span with nested `ask_ldap`, `ask_servicenow`, `create_job`, `upload_file`, `execute_code` tool calls |
| `ldap-agent` | `find_user` tool calls (one per login ID) |
| `servicenow-agent` | `query_table` calls against the `incident` table |
...
- [ ] LDAP data returned for all roster members
- [ ] ServiceNow incidents (or "no open incidents") returned per person
...
| LDAP returns empty results | Bad credentials or login ID not found | Check `LDAP_BIND_DN`/`LDAP_PASSWORD` in `.env`; test with LDAP Agent directly |
| ServiceNow returns empty | Bad credentials or no matching incidents | Check `SERVICENOW_USERNAME`/`SERVICENOW_PASSWORD`; try a broader query |
...
This exercises the LDAP agent's `get_manager_chain` tool.

### Variation C — ServiceNow catalog requests
Change the prompt: *"Instead of incidents, find any catalog requests (RITMs) opened by each person in the last 90 days."*
This exercises the ServiceNow agent's `search_catalog_requests` tool.
