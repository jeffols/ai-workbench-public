...
- **MCP tools as capabilities** — data servers (`ldap-mcp`, `servicenow-mcp`)
  expose narrow HTTP APIs; agents orchestrate them via ReAct loops
- **Specialist agents own domain logic** — `code-agent`, `ldap-agent`,
  `servicenow-agent`, `enrichment-agent` each embed workflow knowledge
...
- **Tool reusability** — some tools are generic (`open-terminal`,
  `magic-fetch`), but enterprise adapters (`ldap-mcp`, `servicenow-mcp`)
