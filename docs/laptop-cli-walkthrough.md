...
```
$ codex "ask directory-agent who reports to Jane Doe"
# → runs on your laptop, reasons via Azure (through LiteLLM),
#   calls directory-agent → directory-mcp → directory service, renders answer.
...
your config (`directory-agent`, `ticketing-agent`, `code-agent`,
`enrichment-agent`, `directory-mcp`, `ticketing-mcp`, `magic-fetch`).
...
```
use directory-agent to find everyone in the Data_Science group
```

You should see Codex decide to call `directory-agent.org_query(...)`, which
spins the workbench's ReAct loop (directory-agent → directory-mcp → directory service), then
render the answer.

Open `http://phoenix.localhost` in a browser. Under the `directory-agent`
...
the directory and their open the ticketing system incidents. Export the result as xlsx.
```

What should happen: Codex calls `enrichment-agent.enrich_data(task=...)`
with the CSV inline. The enrichment agent delegates to `directory-agent` and
`ticketing-agent`, generates Python that iterates the rows via mcpo,
...
- Enterprise adapters (`directory-mcp`, `ticketing-mcp`) are **read-only until
...
`docker compose restart directory-agent`.
...
1. **Decide on the k8s scaffold.** Start a `charts/workbench/` directory
   ...
   P0-3, P1-1, P1-3 land.
2. **Pick P1-1 (LangGraph migration) for the `directory-agent` pilot.**
